<template>
    ...
    <div class="_wrap"></div>
      ...
      <div class="login_type" v-if="logintype.type !== '注册'">
        ...
        <div class="login-img"><img :src="cdnPath + 'loginimg.png'" @click="changetype"/></div>
      </div>
      <!-- 二维码登录 -->
	  	<div class="logind" v-if="logintype.type == '短信验证登录'">
            ...
            <div class="cont_help">
                <div class="cont_help_left"><span>?</span></div>
          ...
          <div class="bottom_text bottom_bnt">没有账号 <span @click="changetypeto">立即注册</span></div>
	  		</div>
	  	</div>
      ...
      <div class="login_button" :class="{ login_butto: loginto.captcha }" @click="logintoo(
      loginto.mobile,
      loginto.cValue,
      registered.entrance,
      ...
      captchaimg.captchaValue)">
			...
			<div class="bottom_text">没有账号 <span @click="changetypeto">立即注册</span></div>
		</div>
      ...
      <div class="login_button" :class="{ login_butto: registered.captcha }" @click="logintoo(
      registered.mobile,
      registered.cValue,
      registered.entrance,
      captchaimg.captchaKey,
      captchaimg.captchaValue)">
        <span>注册</span>
	  		...
	  		<div class="bottom_text">已有账号 <span @click="changetypeto">马上登录</span></div>
	  	</div>
	  </div>
...
<script>
  ...
  import { saveAccessToken } from '@/api/cacheService'
    ...
    // 登录方式切换
    changetype () {
      this.logintype.type = this.logintype.type == '二维码登录' ? '短信验证登录' : '二维码登录'
    }
    //短信登录注册切换
    changetypeto () {
      this.logintype.type = this.logintype.type == '二维码登录' || this.logintype.type == '短信验证登录' ? '注册' : '二维码登录'
      this.getCaptcha()
    }
    ...
    fromPop(text){
      ...
      if(this.formalert.text == '短信验证码发送成功！'||this.formalert.text == '操作成功！'){ this.formalert.alertstyle = true }
