<template>
  <div id="apply">
    <!-- 未上传 -->
    <div class="resume_main no_upload" v-if="!isUpload" :style="{ height: contentHeight + 'px' }">
      <div class="main_cont">
        ...
        <p class="hint2">附件简历类型支持pdf、jpg、png、doc、docx； 最大允许上传10M；</p>
      </div>
    ...
    <div class="resume_main" v-else :style="{ height: contentHeight2 + 'px' }">
      <div class="file_cont" v-if="uploadFileData[0]">
        ...
        <div class="file_msg">
          ...
          <div class="file_center">
            <p class="file_name">{{uploadFileData[0].fileName}}</p>
      ...
      <div
        class="upload_hint"
        @mouseover="upload_hint_pop = true"
        @mouseout="upload_hint_pop = false"
      >
        上传简历帮助
        <span>?</span>
        <div class="upload_hint_pop" v-if="upload_hint_pop">
          <p>上传简附件，招聘官既可查看你的附件简历；</p>
          <p>附件简历类型支持pdf、jpg、png、doc、docx；</p>
          <p>允许最大上传10M；若从其他平台下载的word简历，请将文件另存为.docx格式后上传。</p>
    ...
    <div class="pop" v-if="pop.isShow">
      <div class="messageBox" v-if="pop.type==='delete'">
        ...
        <div class="btns">
          ...
          <div class="btn true" @click="todoAction('delete')">删除</div>
        </div>
      </div>
    </div>
  </div>
